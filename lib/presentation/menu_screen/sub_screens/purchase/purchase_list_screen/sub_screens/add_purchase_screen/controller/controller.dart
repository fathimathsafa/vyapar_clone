...
import 'package:vyapar_clone/model/bill_no_model.dart';
import 'package:vyapar_clone/presentation/menu_screen/sub_screens/purchase/purchase_list_screen/model/purchase_model.dart';
...
class AddPurchaseController extends GetxController {
 ...
 RxString saleId = ''.obs;
  ...
  var billNoModel = BillNoModel().obs;
  var saleDetailModel = SaleDetailModel().obs;
  RxList itemList = <ItemModel>[].obs;
  RxList<PurchaseListModel> purchaseList = <PurchaseListModel>[].obs;
  ...
  void fetchInvoicNo() async {
    var response = await _apiServices.getRequest(
        endurl: EndUrl.getBillNo,
        authToken: await SharedPreLocalStorage.getToken());

    if (response != null) {
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        BillNoModel ob = BillNoModel(
            id: response.data['data']['_id'],
            billNo: response.data['data']['purchaseBillNo']);
        billNoModel.value = ob;
      }
    }
  }
  ...
  String saleValidator() {
    ...
    } else if (billNoModel.value.billNo == null) {
      SnackBars.showErrorSnackBar(text: "Empty bill number");
  ...
  void addSale() async {
    ...
    dio.FormData formData = dio.FormData.fromMap({
      'billNumber': billNoModel.value.billNo.toString(),
      ...
      'paidAmount': double.parse(recivedAmountController.text)
          .toStringAsFixed(2)
          .toString(),
    ...
    if (response != null) {
      ...
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        ...
        fetchInvoicNo();
        Get.snackbar("Successfully added purchase", "",
            backgroundColor: Colors.green);
        SnackBars.showSuccessSnackBar(text: "Successfully added purchase");
        // homec.getAllInvoice();
        setLoadingValue(false);
  ...
  void updateSale() async {
    ...
    dio.FormData formData = dio.FormData.fromMap({
      'invoiceNo': billNoModel.value.billNo.toString(),
      'invoiceType': selectedSaleType.value.toString(),
 ...
 if (response != null) {
  setLoadingValue(false);
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        ...
        List<PurchaseListModel> list = List<PurchaseListModel>.from(
            jsonResponse.map((x) => PurchaseListModel.fromJson(x)));

        purchaseList.assignAll(list);

        printInfo(info: "purchaseList list ==${purchaseList.length}");

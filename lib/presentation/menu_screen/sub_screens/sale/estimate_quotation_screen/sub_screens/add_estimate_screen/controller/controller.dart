...
class AddEstimateController extends GetxController {
  ...
  var referenceModel = ReferenceNoModel().obs;
  //addItem
  ...
  void fetchRefferenceNo() async {
    var response = await _apiServices.getRequest(
        endurl: EndUrl.getReferencNo,
        authToken: await SharedPreLocalStorage.getToken());

    if (response != null) {
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        ReferenceNoModel ob = ReferenceNoModel(
            id: response.data['data']['_id'],
            quotationReferenceNo: response.data['data']
                ['quotationReferenceNo']);
        referenceModel.value = ob;
      }
    }
  }
  ...
  String saleValidator() {
    ...
    } else if (referenceModel.value.quotationReferenceNo == null) {
      SnackBars.showErrorSnackBar(text: "Empty reference number");
  ...
  void addSale() async {
    ...
    dio.FormData formData = dio.FormData.fromMap({
      ...
      'partyId': customerTxtCont.text,

      'referenceNo': referenceModel.value.quotationReferenceNo.toString(),
    ...
    var response = await _apiServices.postMultiPartData(
        ...
        files: fileList,
        endUrl: EndUrl.quotationUrl,
        authToken: await SharedPreLocalStorage.getToken());

    if (response != null) {
      printInfo(info: "response to save invoice==$response");
      if (response.statusCode!.toInt()==200 || response.statusCode!.toInt()==201) {
        // SnackBars.showSuccessSnackBar(text: "Successfully saved quotation");
        Get.snackbar("Successfully saved quotation", "",backgroundColor: Colors.green);

...
import '../../../../../../../../model/order_no_model.dart';
...
class AddSaleController extends GetxController {
  ...
  var invoiceNo = OrderNoModel().obs;
  RxList itemList = <ItemModel>[].obs;
  RxList saleOrderList = <SaleOrderModel>[].obs;
  ...
  void fetchInvoicNo() async {
    var response = await _apiServices.getRequest(
        endurl: EndUrl.orderNo,
        authToken: await SharedPreLocalStorage.getToken());

    if (response != null) {
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        OrderNoModel ob = OrderNoModel(
 ...
 if (response != null) {
  ...
  void fetchSaleOrderList() async {
     setLoadingValue(true);
    var response = await _apiServices.getRequest(
        endurl: EndUrl.saleOrder,
        authToken: await SharedPreLocalStorage.getToken());
 if (response != null) {
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
        var jsonResponse = response.data['data'];

        List<SaleOrderModel> list = List<SaleOrderModel>.from(
            jsonResponse.map((x) => SaleOrderModel.fromJson(x)));

        saleOrderList.assignAll(list);
       printInfo(info: "sale order list =${saleOrderList.length}");
        setLoadingValue(false);
      }
      setLoadingValue(false);
...
String saleValidator(){
  ...
  }else if(invoiceNo.value.orderNo==null){
    SnackBars.showErrorSnackBar(text: "Empty order number");
...
void addSale()async{
  ...
  for (int i = 0; i < itemList.length; i++) {
   ...
   printInfo(info: "order no ===${invoiceNo.value.orderNo}");

  dio.FormData formData = dio.FormData.fromMap({

    'orderNo': invoiceNo.value.orderNo.toString(),
    // 'invoiceType': selectedSaleType.value.toString(),
    'orderDate': selectedSaleDate.value.toString(),
    'party': 'AK Traders',
    ...
    'advanceAmount': double.parse(recivedAmountController.text).toStringAsFixed(2).toString(),
  ...
  var response = await _apiServices.postMultiPartData(
    ...
    files: fileList,
      ...
      if (CheckRStatus.checkResStatus(statusCode: response.statusCode)) {
         ...
         fetchInvoicNo();
         Get.snackbar("Successfully saved sale order", "",backgroundColor: Colors.green);
         SnackBars.showSuccessSnackBar(text: "Successfully saved sale order");
        //  homec.getAllInvoice();
        setLoadingValue(false);
